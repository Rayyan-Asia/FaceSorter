...
Usage:
    python extract_embedding.py <image_path>
...
def main():
    if len(sys.argv) != 2:
        print(json.dumps({"error": "Usage: python extract_embedding.py <image_path>"}), file=sys.stderr)
        sys.exit(1)
    ...
    try:
        ...
        logger.error("Failed to extract embedding from %s: %s", image_path, e)
